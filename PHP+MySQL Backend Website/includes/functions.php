...
function updateHotelCoordination($hotelId, $status, $contactPersonName, $contactPhone, $contactEmail) {
    query(
        'UPDATE hotels SET status = ?, contact_person_name = ?, contact_phone = ?, contact_email = ? WHERE hotel_id = ?',
        [$status, $contactPersonName, $contactPhone, $contactEmail, $hotelId]
    );
}
...
function updateTransportCoordination($transportId, $status, $contactName, $contactPhone) {
    query(
        'UPDATE transport SET status = ?, contact_name = ?, contact_phone = ? WHERE transport_id = ?',
        [$status, $contactName, $contactPhone, $transportId]
    );
}
...
function countPackageReferences($packageId) {
    ...
    if (tableExists('bookings')) {
        $stmt = query('SELECT COUNT(*) AS total FROM bookings WHERE package_id = ?', [$packageId]);
        $result = $stmt->get_result();
        $total += intval($result->fetch_assoc()['total'] ?? 0);
    }

    if (tableExists('reviews')) {
        $stmt = query('SELECT COUNT(*) AS total FROM reviews WHERE package_id = ?', [$packageId]);
        $result = $stmt->get_result();
        $total += intval($result->fetch_assoc()['total'] ?? 0);
    }
...
function deletePackage($packageId) {
    if (countPackageReferences($packageId) > 0) {
        return false;
    }
    ...
    if (tableExists('package_gallery')) {
        query('DELETE FROM package_gallery WHERE package_id = ?', [$packageId]);
    }

    query("DELETE FROM `$table` WHERE `$idColumn` = ?", [$packageId]);
